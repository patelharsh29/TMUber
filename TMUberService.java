...
abstract public class TMUberService 
{
  ...
  {
    // Fill in the code
    if (!(other instanceof TMUberService)) {return false;}        // checking if object parameter is instance of TMUberService class
    TMUberService otherD = (TMUberService) other;                  // if it is then initalizing it 
    return ((this.type.equals(otherD.type)) && (this.user.equals(otherD.user)));    // checking if the type and user are the same, if they are then returns true else false
  }
  
  // Print Information 
  public void printInfo()
  {
    System.out.printf("Type: %-9s From: %-15s To: %-15s", type, from, to);
    System.out.print("\nUser: ");
    user.printInfo();
  }
}
